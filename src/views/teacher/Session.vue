<template>
  <div class="max-w-screen-md mx-auto px-4 py-10">
    ...
    <div class="p-8 flex items-start bg-dark-grey rounded-md shadow-lg">
      <form class="flex flex-col gap-y-4 w-full">
        ...
        <div class="flex flex-col">
          <label for="date" class="text-sm text-light-grey"> Date </label>
          <input
            id="date"
            v-model="date"
            type="date"
        ...
        <!-- TEACHER -->
        <div class="flex flex-col">
          <label for="teacher" class="text-sm text-light-grey"> Teacher </label>
          <select
            id="teacher"
            v-model="teacher"
            required
        ...
        <!-- TOPIC -->
        <div class="flex flex-col">
          <label for="topic" class="text-sm text-light-grey"> Topic </label>
          <select
            id="topic"
            v-model="topic"
            class="p-2 text-gray-100 focus:outline-transparent bg-med-grey rounded-sm"
    ...
    <!-- ATTENDANCE LIST -->
    <div
      class="p-4 bg-dark-grey rounded-md shadow-lg flex flex-col justify-center mt-4"
    >
      <Switcher
        class="flex justify-center max-w-screen-sm py-1 px-2 mb-4"
        :title-left="'Active'"
        :title-right="'Inactive'"
      />
      <div class="mb-2 self-center">
        <span class="text-2xl flex text-m text-light-grey"
          >Attendance - {{ humanIdList.length - 1 }}</span
        >
      </div>
      <form
      ...
      >
        ...
        <div
          v-if="activeCard"
          :key="cardRerenderKey"
          class="items-center text-light-grey text-md"
        >
          <MultiCheckbox
    ...
    <div
      class="p-5 bg-dark-grey rounded-md shadow-lg flex flex-col justify-center mt-4"
    >
      <div class="mb-2 self-center">
      ...
      <div class="flex pl-4 px-6 justify-center">
        <ul class="list-inside space-y-1 justify-center">
          <li class="text-l text-light-grey -mb-1">
            📅 {{ latestSessionSavedDate }}
          </li>
          <li class="text-l text-light-grey">
            📝 {{ latestSessionSavedTopic }}
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const fizzID = '630fbcd603c610ee444351b5'
    teacher.value = carlosCampoyID
    const carlosIdObject = { id: carlosCampoyID }
    const fizzIdObject = { id: fizzID }
    ...
    const date = ref(null)
    const humanIdList = ref([carlosIdObject, fizzIdObject]) // used in multicheckbox, has Carlos Campoy as default value to keep track of classes (teacher always attends)
    ...
    const createAttendanceLists = async () => {
      activeAttendanceList.value = await createAttendanceArray(
        toRaw(activeHumans)
      )
      inactiveAttendanceList.value = await createAttendanceArray(
        toRaw(inactiveHumans)
      )
    }
    ...
    async function displayLatestSessionSaved() {
      ...
      latestSessionSavedDate.value = new Date(
        latestLessonDate
      ).toLocaleDateString()
      latestSessionSavedTopic.value = focusLessons.filter((lesson) =>
        lesson.id.includes(latestLessonId)
      )[0].name
    }
