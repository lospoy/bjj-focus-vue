<template>
  <div
    class="max-w-screen-sm mx-auto px-4 py-10 text-light-grey bg-dark-grey shadow-lg"
  >
    <!-- Error Handling -->
    <div v-if="errorMsg" class="mb-10 p-4 rounded-md">
      <p class="text-red-500">{{ errorMsg }}</p>
    </div>

    <!-- NewTechnique -->
    <form @submit.prevent="newTechnique" class="p-8 flex flex-col rounded-md">
      <h1 class="text-3xl mb-4 self-center">New Technique</h1>

      <div class="flex flex-col mb-2">
        <label for="position" class="mb-1 text-sm">Position</label>
        <input
          type="text"
          required
          class="p-2 bg-med-grey focus:outline-none"
          id="position"
          v-model="position"
        />
      </div>
      ...
      <div class="flex flex-col mb-2">
        <label for="move" class="mb-1 text-sm">Move</label>
        <div class="flex-row">
          <input
            type="text"
            required
            class="p-2 bg-med-grey focus:outline-none w-4/6"
            id="move"
            v-model="move"
          />
          <select
            required
            class="p-2 bg-med-grey focus:outline-none w-2/6"
            id="moveCategory"
      ...
      <div class="flex flex-col mb-2">
        <label for="variation" class="mb-1 text-sm">Variation</label>
        <input
          type="text"
          class="p-2 bg-med-grey focus:outline-none"
          id="variation"
      ...
      <div class="flex flex-col mb-2">
        <label for="notes" class="mb-1 text-sm">Notes</label>
        <input
          type="text"
          class="p-2 bg-med-grey focus:outline-none"
          id="notes"
  ...
  <div class="max-w-screen-sm mx-auto px-4 py-10 text-light-grey">
    <div class="p-8 flex flex-col rounded-md shadow-lg">
      ...
      <div class="flex flex-col mb-6">
        ...
        <ul id="passList">
          <li v-for="(item, index) of passList" :key="index">- {{ item }}</li>
        </ul>
      </div>

      <div class="flex flex-col mb-6">
        <label for="entryList" class="mb-1 text-xl">Entry</label>
        <ul id="entryList">
          <li v-for="(item, index) of entryList" :key="index">- {{ item }}</li>
        </ul>
      </div>

      <div class="flex flex-col mb-6">
        <label for="escapeList" class="mb-1 text-xl">Escape</label>
        <ul id="escapeList">
          <li v-for="(item, index) of escapeList" :key="index">- {{ item }}</li>
        </ul>
      </div>

      <div class="flex flex-col mb-6">
        <label for="submissionList" class="mb-1 text-xl">Submission</label>
      ...
      <div class="flex flex-col mb-6">
        <label for="sweepList" class="mb-1 text-xl">Sweep</label>
        <ul id="sweepList">
          <li v-for="(item, index) of sweepList" :key="index">- {{ item }}</li>
        </ul>
      </div>

      <div class="flex flex-col mb-6">
        <label for="takedownList" class="mb-1 text-xl">Takedown</label>
        <ul id="takedownList">
...
import {
  createTechnique,
  getAllTechniques
} from '../../services/bjj_services/techniqueService'
import {
  createPosition,
  getAllPositions,
  getPosition
} from '../../services/bjj_services/positionService'
import {
  createMove,
  getAllMoves,
  getMove
} from '../../services/bjj_services/moveService'
import {
  createVariation,
  getAllVariations,
  getVariation
} from '../../services/bjj_services/variationService'
  ...
  setup() {
    ...
    const decryptTechnique = async (positionId, moveId, variationId) => {
      ...
      if (moveObject.category.pass === true) {
        passList.push(
          `${variationObject.name.english} ${moveObject.name.english} Pass from ${positionObject.name.english}`
        )
      }

      if (moveObject.category.entry === true) {
        entryList.push(
          `${variationObject.name.english} Entry ${moveObject.name.english} from ${positionObject.name.english}`
        )
      }

      if (moveObject.category.escape === true) {
        escapeList.push(
          `${variationObject.name.english} ${moveObject.name.english} Escape from ${positionObject.name.english}`
        )
      }

      if (moveObject.category.submission === true) {
        submissionList.push(
          `${variationObject.name.english} ${moveObject.name.english} from ${positionObject.name.english}`
        )
      }

      if (moveObject.category.sweep === true) {
        sweepList.push(
          `${variationObject.name.english} ${moveObject.name.english} Sweep from ${positionObject.name.english}`
        )
      }

      if (
        moveObject.category.takedown === true &&
        positionObject.name.english === 'Standing'
      ) {
        takedownList.push(
          `${variationObject.name.english} ${moveObject.name.english} Takedown`
        )
      } else if (moveObject.category.takedown === true) {
        takedownList.push(
          `${variationObject.name.english} ${moveObject.name.english} Takedown from ${positionObject.name.english}`
        )
      }
    }
    ...
    const newPosition = async () => {
      ...
      const foundPosition = allPositions.filter(
        (x) => x.name.english.toLowerCase() === position.value.toLowerCase()
      )[0]
    ...
    const newMove = async () => {
      ...
      const foundMove = allMoves.filter(
        (x) => x.name.english.toLowerCase() === move.value.toLowerCase()
      )[0]
    ...
    const newVariation = async () => {
      ...
      const foundVariation = allVariations.filter(
        (x) => x.name.english.toLowerCase() === variation.value.toLowerCase()
      )[0]
    ...
    const newTechnique = async () => {
      ...
      const foundTechnique =
        allTechniques.filter((a) => a.position === positionId)[0] &&
        allTechniques.filter((b) => b.move === moveId)[0] &&
        allTechniques.filter((c) => c.variation === variationId)[0]
